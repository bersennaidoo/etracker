...
func JSONMiddleware(next http.Handler) http.Handler {
	return http.HandlerFunc(func(wr http.ResponseWriter, req *http.Request) {
		...
		if strings.TrimSpace(contentType) == "" {
			...
			if parseError != nil {
				handler.JSONError(wr, http.StatusBadRequest, "Bad or no content-type header found")
				return
			}
		}

		if contentType != "application/json" {
			handler.JSONError(wr, http.StatusUnsupportedMediaType, "Content-Type not application/json")
			return
		}
